...
| Layer | Technology |
|-------|-----------|
| Frontend | React 19 + TypeScript, Vite, react-grid-layout, TanStack Query |
...
1. The React client requests normalized sports data from the `api/` functions (Vercel in production, `scripts/dev-api.ts` locally) through the TanStack Query hooks in `client/src/api/queries.ts`. Components asking for the same data share one request and one cached response; scores poll every 30 s (live box scores too), polling pauses while the tab is hidden, and data refetches when the tab becomes visible.
